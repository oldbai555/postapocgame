...
- 🆕 **阶段六：Legacy EntitySystem 清理**：删除 `server/service/gameserver/internel/playeractor/entitysystem` 下 Bag/Money/Level/Skill/Quest/Fuben/ItemUse/Shop/Attr/Equip 等旧实现，所有调用统一改用 `adapter/system` + UseCase/Controller，避免 Legacy 入口与循环依赖
...
- [⏳] **阶段五：辅助系统重构**
  ...
  - [ ] VipSys（VIP 系统）——已新增 `domain/vip/vip.go` 与 `usecase/vip/vip_money_use_case.go`，通过 `AddMoneyUseCase` + `VipMoneyUseCaseImpl` 统一处理 `MoneyTypeVipExp`，并将 VipSys 迁移为 `adapter/system/vip`，保留特权查询与兼容接口
  - [ ] DailyActivitySys（日常活跃度系统）——已新增 `domain/dailyactivity` 与 `usecase/dailyactivity`，通过 `AddMoneyUseCase/ConsumeMoneyUseCase` + `PointsUseCase` 统一处理活跃点增减，迁移旧 `DailyActivitySys` 为 SystemAdapter 并删除 legacy 实现
  - [ ] MessageSys（玩家消息系统）——已将旧 `MessageSys` 生命周期迁移到 `adapter/system/message`，保留 `DispatchPlayerMessage` 作为统一回放入口，后续仅需在有新业务场景时扩展消息类型与回调
  - [✅] GMSys（GM 系统，GM 命令与工具函数 Clean Architecture 化）——新增 `adapter/system/gm`（GMSystemAdapter + GMManager + GM 工具函数），在系统适配器中注册 `C2SGMCommand` 协议处理，旧 `entitysystem/gm_sys.go` 与 `gm_manager.go` 已删除，仅保留 `entitysystem/gm_tools.go` 作为背包满奖励等场景的系统邮件兼容入口

- [⏳] **阶段六：清理与优化**
  - Legacy EntitySystem 移除进度：
    - [✅] RecycleSys：删除 `server/service/gameserver/internel/playeractor/entitysystem/recycle_sys.go`，客户端协议与 DungeonServer 回写链路全部由 `adapter/system/recycle` + `RecycleController` 托管
    - [✅] BagSys / MoneySys / LevelSys / SkillSys / QuestSys / FubenSys / ItemUseSys / ShopSys / AttrSys / EquipSys：逐一确认所有调用切换到 SystemAdapter 与 UseCase 后再删除 legacy `*_sys.go`
  - [⏳] 完善测试
  - [⏳] 文档更新

### 6.2 Phase 2 核心玩法（进行中）
...
- **协议注册规范**：
  - **GameServer**：协议注册统一在 `entitysystem` 各业务系统的 `init()` 中，通过 `gevent.Subscribe(gevent.OnSrvStart, ...)` 订阅服务器启动事件，在事件回调中调用 `clientprotocol.Register` 注册协议处理器
  - **DungeonServer**：协议注册统一在 `entitysystem` 各业务系统的 `init()` 中，通过 `devent.Subscribe(devent.OnSrvStart, ...)` 订阅服务器启动事件，在事件回调中调用 `clientprotocol.Register` 注册协议处理器；禁止在 `clientprotocol` 目录下直接注册协议，所有协议注册应归到对应的业务系统中
- **Controller 初始化**：从阶段六起，所有 GameServer 协议/ RPC 注册逻辑统一放在 `adapter/controller/*_controller_init.go`，SystemAdapter 只负责工厂与生命周期，避免 System Package ↔ Controller 的循环依赖
...
- Friend/Blacklist 数据统一存储在 `PlayerRoleBinaryData.FriendData` 与 `BlacklistRepository`，任何模块需要访问必须依赖对应 Use Case 或接口，不得直接操作数据库
...
- `server/service/gameserver/internel/playeractor/entitysystem/*`：legacy 玩家子系统（仍包含 AntiCheat/GMSys/FubenSys/AttrSys/MessageSys 等未迁移模块）
- `server/service/gameserver/internel/playeractor/entity/player_network.go`：客户端协议处理入口
- `server/service/gameserver/internel/playeractor/entity/player_role.go`：PlayerRole 主体逻辑与 `sendPublicActorMessage` 封装，统一通过 `PublicActorGateway` 发送消息
