...
- **PlayerActor 瘦身（2025-12-10）**
  - 移除 `di/` 与 `adapter/context/`，统一依赖装配到 `adapter/deps`，Context 取值收敛到 `core/gshare/context_helper.go`，`go test ./service/gameserver/internel/app/playeractor/...` 通过。
  - 合并 Network/Session Gateway 为单一 `ClientGateway`（`adapter/gateway/client_gateway.go`），统一由 `deps.ClientGateway()` 提供，会话访问与发送复用一套实现。
  - 收敛通用消耗/奖励适配：删除 `adapter/usecaseadapter`，在 UseCase 层新增 `usecase/consume`、`usecase/reward` 基于 Bag/Money 用例实现，Quest/Fuben/Shop/Skill 等直接注入 UseCase，保持依赖倒置。
  - Controller 注册与 PlayerActorMsg 路由集中到 `adapter/controller/register_all.go`，遗留的 EnterGame/QueryRank/PlayerActor 消息处理迁移到 `adapter/controller/player_network_controller.go`；玩家事件总线改为按 Actor 构造 `gevent.NewPlayerEventBus()`，无全局模板克隆。
  ...
  - 背包系统瘦身：索引/快照逻辑集中到 `usecase/bag/accessor.go`，SystemAdapter 仅做生命周期与路由，Add/Remove/HasItem 全量走 UseCase。
- **统一时间与日志**
...
- C2S/S2C：Controller 注册集中在 `adapter/controller/register_all.go`，禁止在 SystemAdapter 和 entitysystem 里散落注册；DungeonActor 也不再直接注册或处理任何 C2S 协议。
...
- （2025-12-10 更新）注册/登录/角色列表/创角协议已拆到 `PlayerAccountController`、`PlayerRoleController`，EnterGame/QueryRank/PlayerActorMsg 入口收敛到 `player_network_controller.go` 并在 `register_all.go` 统一注册；玩家事件总线由 `gevent.NewPlayerEventBus()` 按 Actor 构造，无全局模板克隆。
- PlayerActor 依赖注入：已删除 `di/` 与 `adapter/context/`，统一通过 `adapter/deps` 暴露的依赖或函数装配，Context 取值使用 `core/gshare/context_helper.go`。
...
- 背包系统不再在 SystemAdapter 内维护索引/快照，统一由 `usecase/bag/accessor.go` 负责索引重建、快照与恢复，SystemAdapter 只路由 UseCase。
...
- 玩家网络入口：`internel/app/playeractor/adapter/controller/player_network_controller.go`（注册集中于 `adapter/controller/register_all.go`）。
...
- UseCase 与 SystemAdapter 瘦身点：`usecase/equip/*`、`usecase/item_use/use_item.go`、`usecase/fuben/*`、`adapter/system/{equip,fuben,item_use}_system_adapter.go`。
  - 背包索引/快照 helper：`usecase/bag/accessor.go`（统一索引重建、快照/恢复，SystemAdapter 不再维护缓存）。
