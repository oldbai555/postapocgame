...
| 组件 | 作用 | 关键文件 |
| ---- | ---- | -------- |
| `AdventurePanel` | 文本面板、命令解析、状态机（登录/进场景/战斗）。 | `server/example/adventure_panel.go` |
| `ClientManager` | 复用 ActorManager，为每个客户端建 Actor。 | `server/example/client_manager.go` |
| `GameClient` | 网络编解码、协议流、AOI/技能回包监听。 | `server/example/game_client.go` |
| `ClientHandler` | Actor 消息处理 → `GameClient` 回调。 | `server/example/client_handler.go` |
...
| `move <dx> <dy>` | `C2SStartMove/C2SEndMove` | 自动生成 seq、封装移动速率。 |
...
| 编号 | 内容 | 说明 |
| ---- | ---- | ---- |
| GCLI-1 | 背包/物品指令 | `bag`, `use-item`, `loot` 与 `ItemSys` 协议映射。 |
| GCLI-2 | 养成面板 | 等级/属性/任务/活跃度等查询与操作面板。 |
| GCLI-3 | 战斗脚本化 | 预设战斗脚本（巡逻/技能循环）、统计命中/伤害。 |
| GCLI-4 | 副本/匹配流程 | 接入 `FubenSys`、多人副本匹配、战斗录像触发。 |
| GCLI-5 | GM/调试命令桥接 | 面板命令映射至 GM 协议，支持批量调试。 |
| GCLI-6 | 协议回放/录制 | 将面板输入输出保存为脚本，供回归测试复用。 |
