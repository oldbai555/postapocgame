...
**配置说明**: 定义任务及其目标，任务的所有公共枚举与结构在 `proto/csproto/quest_def.proto` 中定义，客户端与服务端共享。

**示例配置**（与 `server/internal/jsonconf/quest_config.go` 对齐）:
```json
{
  "quests": [
    {
...
**字段说明**:
- `questId`: 任务ID（与 `QuestData.id` 对应，通过该ID从配置表反查任务内容与类型）
- `name`: 任务名称
- `type`: 任务大类，使用 `QuestCategory` 枚举（1=主线，2=支线，3=日常，4=周常），定义在 `quest_def.proto` 中
- `description`: 任务描述
- `level`: 需求等级
- `preQuests` / `nextQuests`: 前置/后续任务链
- `targets`: 任务目标数组
  - `type`: 任务目标类型，使用 `QuestType` 枚举（1=和NPC对话，2=学习任意技能，3=击杀任意怪物），定义在 `quest_def.proto` 中
  - `ids`: 条件数组（整形数组，对于对话任务配置 npcId，部分类型可为空表示“任意”）
  - `count`: 达标数量
- `rewards`: 完成任务发放的基础道具奖励（数组，字段结构见 `ItemSt`）
- `expReward`: 经验奖励
- `autoTrack`: 是否自动追踪
- `maxCount`: 可重复完成次数（日常/周常任务用；0 表示不限，受 QuestSys 的 `QuestFinishCount` 逻辑约束）
- `activePoint`: 完成后奖励的活跃点（日常任务用，驱动日常活跃系统）

> 实现细节：服务端 `QuestData` 结构中只保留 `id` 与 `progress`，不再冗余存储任务类型与时间字段；任务类型与大类一律通过 `questId` → 配表推导，保证客户端与服务端通过 csproto 共享同一份定义。
...
9. **公共枚举统一入口**: 所有服务端/客户端共享的枚举（任务枚举、实体类型、错误码、战斗状态等）统一放在 `proto/csproto/*.proto` 中维护，禁止在 Go / C# 代码中各自定义“魔法数字”；修改后通过 `genproto` 分别生成 `server/internal/protocol` 与客户端 C# 协议代码。
