...
type EntityMgr struct {
	entities     map[uint64]iface2.IEntity // hdl -> entity
	sessions     map[string]uint64         // sessionId -> entity hdl
	entityScenes map[uint64]iface2.IScene
}
...
func GetEntityMgr() *EntityMgr {
	if globalEntityMgr == nil {
		globalEntityMgr = &EntityMgr{
			...
			entityScenes: make(map[uint64]iface2.IScene),
		}
	}
...
// Register 注册实体
func (m *EntityMgr) Register(entity iface2.IEntity) error {
	hdl := entity.GetHdl()
...
func (m *EntityMgr) GetBySession(sessionId string) (iface2.IEntity, bool) {
	if sessionId == "" {
		return nil, false
	}
...
// BindScene 绑定实体所在场景
func (m *EntityMgr) BindScene(hdl uint64, scene iface2.IScene) {
...
func (m *EntityMgr) GetSceneByHandle(hdl uint64) (iface2.IScene, bool) {
	scene, ok := m.entityScenes[hdl]
	return scene, ok
}

// GetByHdl 通过hdl获取实体
func (m *EntityMgr) GetByHdl(hdl uint64) (iface2.IEntity, bool) {
	entity, ok := m.entities[hdl]
	return entity, ok
}

// GetById 通过Id获取实体列表（可能有多个相同Id的实体）
func (m *EntityMgr) GetById(Id uint64) []iface2.IEntity {
	entities := make([]iface2.IEntity, 0)
...
// GetAll 获取所有实体
func (m *EntityMgr) GetAll() []iface2.IEntity {
	entities := make([]iface2.IEntity, 0, len(m.entities))
	for _, entity := range m.entities {
		entities = append(entities, entity)
	}
	return entities
...
func (m *EntityMgr) Clear() {
	m.entities = make(map[uint64]iface2.IEntity)
	m.entityScenes = make(map[uint64]iface2.IScene)
}
