...
func (ms *MoveSys) MovingTime(mustStop bool) bool {
	...
	speed := ms.speed
	if speed <= 0 {
		speed = uint32(et.GetAttrSys().GetAttrValue(attrdef.Speed))
		if speed <= 0 {
...
func (ms *MoveSys) HandleStartMove(scene iface.IScene, req *protocol.C2SStartMoveReq) error {
	...
	if ret != 0 {
		...
		return customerr.NewErrorByCode(ret, "start move failed")
	}
...
func (ms *MoveSys) LocationUpdate(cPx, cPy int32) bool {
	...
	if speed <= 0 {
		speed = int32(et.GetAttrSys().GetAttrValue(attrdef.Speed))
		if speed <= 0 {
...
func (ms *MoveSys) HandleUpdateMove(scene iface.IScene, req *protocol.C2SUpdateMoveReq) error {
	...
	if speed <= 0 {
		speed = float64(ms.entity.GetAttrSys().GetAttrValue(attrdef.Speed))
		if speed <= 0 {
...
func (ms *MoveSys) BroadcastStopMove(posX, posY uint32, broadcast, sendToSelf bool) {
	if ms.scene == nil {
		return
	}
	...
	if sendToSelf {
		ms.entity.SendMessage(uint16(protocol.S2CProtocol_S2CStopMove), data)
	}
	...
	for _, et := range ms.scene.GetAllEntities() {
		...
		et.SendMessage(uint16(protocol.S2CProtocol_S2CStopMove), data)
	}
}
...
func (ms *MoveSys) StopMove(broadcast, sendToSelf bool) {
	...
	ms.BroadcastStopMove(px, py, broadcast, sendToSelf)
}
...
func (ms *MoveSys) notifyAppear(receiver iface.IEntity, subject iface.IEntity) {
	...
	if receiver.GetEntityType() != uint32(protocol.EntityType_EtPlayer) {
		return
	}
...
func (ms *MoveSys) notifyDisappearByHandle(receiverHdl uint64, subjectHdl uint64) {
	...
	if !ok || target == nil || target.GetEntityType() != uint32(protocol.EntityType_EtPlayer) {
		return
	}
