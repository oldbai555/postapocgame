...
func GetPlayerRoleFromContext(ctx context.Context) (iface.IPlayerRole, error) {
	...
	if value == nil {
		return nil, customerr.NewErrorByCode(int32(protocol.ErrorCode_Internal_Error), "not found %s value", ContextKeyRole)
	}
...
// MustGetPlayerRoleFromContext 从 Context 获取 PlayerRole（失败记录错误并返回 nil）
...
// MustGetSessionIDFromContext 从 Context 获取 SessionID（失败记录错误并返回空字符串）
...
// MustGetRoleIDFromContext 从 Context 获取 RoleID（失败记录错误并返回 0）
