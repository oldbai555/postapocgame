...
// SendRoleList 发送角色列表
func (p *PlayerRolePresenter) SendRoleList(ctx context.Context, sessionID string, result *playerrole.QueryRolesResult) error {
...
func (p *PlayerRolePresenter) SendCreateRoleResult(ctx context.Context, sessionID string, result *playerrole.CreateRoleResult) error {
	var roleData *protocol.PlayerSimpleData
	if result.Role != nil {
		roleData = toProtoRole(result.Role)
	}

	resp := &protocol.S2CCreateRoleReq{
		Job:      0,
		Sex:      0,
		RoleName: "",
	}
	...
	return p.network.SendToSessionProto(sessionID, uint16(protocol.S2CProtocol_S2CCreateRole), resp)
}
