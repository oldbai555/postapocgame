...
func (uc *LearnSkillUseCase) SetDependencies(levelUseCase interfaces.LevelUseCase, consumeUseCase interfaces.ConsumeUseCase) {
...
func (uc *LearnSkillUseCase) Execute(ctx context.Context, roleID uint64, skillId uint32) error {
	...
	// 检查技能是否已学习
	if binaryData.SkillData.SkillMap[skillId] > 0 {
	...
	if !ok || skillConfig == nil {
		return customerr.NewErrorByCode(int32(protocol.ErrorCode_Internal_Error), "invalid skill config type")
	}
	...
	// 学习技能（等级1）
	binaryData.SkillData.SkillMap[skillId] = 1
