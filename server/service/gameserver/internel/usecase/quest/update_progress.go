...
func (uc *UpdateQuestProgressUseCase) Execute(ctx context.Context, roleID uint64, questType uint32, targetId uint32, count uint32) error {
	...
	if binaryData.QuestData == nil || binaryData.QuestData.QuestMap == nil {
		return nil
	}

	for _, bucket := range binaryData.QuestData.QuestMap {
		if bucket == nil {
			continue
		}
		for _, quest := range bucket.Quests {
			...
			questConfigRaw, ok := uc.configManager.GetQuestConfig(quest.Id)
			if !ok {
				continue
			}
			...
			for targetIndex, target := range questConfig.Targets {
				...
				if uc.isQuestCompleted(binaryData.QuestData, quest.Id) {
