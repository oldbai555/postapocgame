...
type ConfigManager interface {
	...
	GetLevelAttrs(level uint32) map[uint32]uint64

	// GetAttrAddRateConfig 获取属性加成配置
	GetAttrAddRateConfig() *jsonconf.AttrAddRateConfig

	// SetEffect 类型别名，用于避免在业务层直接依赖 jsonconf 具体实现
	// （注意：这里只是为了在 iface 包内复用类型，外部依然通过 ConfigManager 接口访问）
}

// SetEffect 是对 jsonconf.SetEffect 的类型别名，便于在接口层使用
type SetEffect = jsonconf.SetEffect
