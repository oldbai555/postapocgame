...
import (
	...
	gameattrcalc "postapocgame/server/service/gameserver/internel/app/playeractor/adapter/system/attrcalc"
	...
	equip2 "postapocgame/server/service/gameserver/internel/app/playeractor/usecase/equip"
...
type EquipSystemAdapter struct {
	*BaseSystemAdapter
	equipItemUseCase   *equip2.EquipItemUseCase
	unEquipItemUseCase *equip2.UnEquipItemUseCase
}

// NewEquipSystemAdapter 创建装备系统适配器
func NewEquipSystemAdapter() *EquipSystemAdapter {
	equipItemUC := equip2.NewEquipItemUseCase(deps.PlayerGateway(), deps.EventPublisher(), deps.ConfigGateway())
	unEquipItemUC := equip2.NewUnEquipItemUseCase(deps.PlayerGateway(), deps.EventPublisher())
