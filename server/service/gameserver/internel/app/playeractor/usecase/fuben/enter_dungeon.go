...
func (uc *EnterDungeonUseCase) SetDependencies(consumeUseCase interfaces2.ConsumeUseCase) {
...
func (uc *EnterDungeonUseCase) Execute(ctx context.Context, roleID uint64, dungeonID uint32, difficulty uint32) error {
	// 获取副本配置
	dungeonCfg := uc.configManager.GetDungeonConfig(dungeonID)
	if dungeonCfg == nil {
	...
	// 获取 BinaryData（共享引用）
	dungeonData, err := uc.playerRepo.GetDungeonData(ctx)
	if err != nil {
