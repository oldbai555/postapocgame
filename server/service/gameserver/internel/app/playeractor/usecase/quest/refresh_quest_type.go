...
func (uc *RefreshQuestTypeUseCase) SetDependencies(levelUseCase interfaces2.LevelUseCase) {
...
func (uc *RefreshQuestTypeUseCase) Execute(ctx context.Context, roleID uint64, questType uint32) error {
	...
	if questData.QuestFinishCount != nil {
		for questId := range questData.QuestFinishCount {
			cfg := uc.configManager.GetQuestConfig(questId)
			if cfg == nil {
				// 配置已删除或异常的任务，直接清理计数
...
func (uc *RefreshQuestTypeUseCase) EnsureRepeatableQuests(ctx context.Context, roleID uint64, questData *protocol.SiQuestData) error {
	now := servertime.Now()
	dailyBucket := uc.ensureBucket(questData, questCategoryDaily)
	if uc.ShouldRefresh(questData, questCategoryDaily, now) || (dailyBucket != nil && len(dailyBucket.Quests) == 0) {
		if err := uc.Execute(ctx, roleID, questCategoryDaily); err != nil {
			return err
		}
	}
	weeklyBucket := uc.ensureBucket(questData, questCategoryWeekly)
	if uc.ShouldRefresh(questData, questCategoryWeekly, now) || (weeklyBucket != nil && len(weeklyBucket.Quests) == 0) {
