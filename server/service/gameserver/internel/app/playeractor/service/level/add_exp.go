...
type AddExpUseCase struct {
	...
	// 依赖其他系统（通过接口注入）
	moneyUseCase  iface.MoneyUseCase  // 必须：用于同步经验货币
	rewardUseCase iface.RewardUseCase // 必须：用于下发升级奖励
}
...
func (uc *AddExpUseCase) SetDependencies(moneyUseCase iface.MoneyUseCase, rewardUseCase iface.RewardUseCase) {
...
func (uc *AddExpUseCase) Execute(ctx context.Context, roleID uint64, exp uint64) error {
	...
	levelUpUC.SetDependencies(uc.moneyUseCase, uc.rewardUseCase)
