...
// EnterDungeonUseCase 进入副本用例（小 service 风格，持有 Deps）
type EnterDungeonUseCase struct {
	deps Deps
}

// NewEnterDungeonUseCase 创建进入副本用例
func NewEnterDungeonUseCase(deps Deps) *EnterDungeonUseCase {
	return &EnterDungeonUseCase{deps: deps}
}

// Execute 执行进入副本用例
func (uc *EnterDungeonUseCase) Execute(ctx context.Context, roleID uint64, dungeonID uint32, difficulty uint32) error {
	// 获取副本配置
	dungeonCfg := uc.deps.ConfigManager.GetDungeonConfig(dungeonID)
	if dungeonCfg == nil {
	...
	// 检查每日进入次数
	record := uc.getDungeonRecord(dungeonData, dungeonID, difficulty)
	if record != nil {
		now := servertime.Now()
	...
	if len(difficultyCfg.ConsumeItems) > 0 {
		if uc.deps.ConsumeUseCase != nil {
			// 检查消耗是否足够
			if err := uc.deps.ConsumeUseCase.CheckConsume(ctx, roleID, difficultyCfg.ConsumeItems); err != nil {
				return customerr.NewErrorByCode(int32(protocol.ErrorCode_Internal_Error), "消耗物品不足: %v", err)
			}
			// 扣除消耗
			if err := uc.deps.ConsumeUseCase.ApplyConsume(ctx, roleID, difficultyCfg.ConsumeItems); err != nil {
	...
	// 更新进入记录
	if err := uc.updateEnterRecord(dungeonData, dungeonID, difficulty); err != nil {
		return err
	}
