...
func NewConfigGateway() iface.ConfigManager {
...
func (g *ConfigGatewayImpl) GetLevelAttrs(level uint32) map[uint32]uint64 {
	configMgr := jsonconf.GetConfigManager()
	return configMgr.GetLevelAttrs(level)
}

// GetAttrAddRateConfig 获取属性加成配置
func (g *ConfigGatewayImpl) GetAttrAddRateConfig() *jsonconf.AttrAddRateConfig {
	configMgr := jsonconf.GetConfigManager()
	return configMgr.GetAttrAddRateConfig()
}
