...
type BagSystemAdapter struct {
	*sysbase.BaseSystem
	deps              Deps // Phase 2D: 保存依赖，避免全局 deps 调用
	addItemUseCase    *AddItemUseCase
	removeItemUseCase *RemoveItemUseCase
	hasItemUseCase    *HasItemUseCase // TODO(adapter-phaseA-A2): 纯校验型逻辑后续可统一通过 BagUseCase 接口暴露，SystemAdapter 只做路由
}

// NewBagSystemAdapter 创建背包系统适配器
func NewBagSystemAdapter(rt *runtime.Runtime) *BagSystemAdapter {
	d := depsFromRuntime(rt)
	return &BagSystemAdapter{
		BaseSystem:        sysbase.NewBaseSystem(uint32(protocol.SystemId_SysBag)),
		deps:              d,
...
func (a *BagSystemAdapter) OnInit(ctx context.Context) {
	// 初始化可选：提前构建索引用于查询
	_, _ = NewBagSnapshot(ctx, a.deps.PlayerRepo)
}
...
func (a *BagSystemAdapter) GetItem(ctx context.Context, itemID uint32) (*protocol.ItemSt, error) {
	acc, err := NewBagSnapshot(ctx, a.deps.PlayerRepo)
	if err != nil {
		return nil, err
	}
...
func (a *BagSystemAdapter) GetAllItems(ctx context.Context) ([]*protocol.ItemSt, error) {
	bagData, err := a.deps.PlayerRepo.GetBagData(ctx)
	if err != nil {
		return nil, err
	}
...
// 注册系统工厂和协议
// RegisterSystemFactory 注册背包系统工厂（由 register.RegisterAll 调用）
