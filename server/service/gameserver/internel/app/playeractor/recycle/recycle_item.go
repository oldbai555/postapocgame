...
// RecycleItemUseCase 回收物品用例（小 service 风格，持有 Deps）
type RecycleItemUseCase struct {
	deps Deps
}

// NewRecycleItemUseCase 创建回收物品用例
func NewRecycleItemUseCase(deps Deps) *RecycleItemUseCase {
	return &RecycleItemUseCase{deps: deps}
}
...
func (uc *RecycleItemUseCase) Execute(ctx context.Context, roleID uint64, itemID uint32, count uint32) ([]*protocol.ItemAmount, error) {
	...
	if uc.deps.BagUseCase == nil || uc.deps.RewardUseCase == nil {
	...
	// 检查背包中是否有足够数量
	hasItem, err := uc.deps.BagUseCase.HasItem(ctx, roleID, itemID, count)
	if err != nil {
		return nil, err
	}
	if !hasItem {
	...
	// 获取回收配置
	recycleConfig := uc.deps.ConfigManager.GetItemRecycleConfig(itemID)
	...
	if len(jsonconfAwards) > 0 {
		if err := uc.deps.RewardUseCase.GrantRewards(ctx, roleID, jsonconfAwards); err != nil {
	...
	// 删除物品
	if err := uc.deps.BagUseCase.RemoveItem(ctx, roleID, itemID, count); err != nil {
		return nil, customerr.Wrap(err)
	}
