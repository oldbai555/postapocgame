...
func (d *DungeonActor) AsyncCall(ctx context.Context, sessionId string, msgId uint16, data []byte) error {
	...
	if err := d.actorMgr.SendMessageAsync("global", msg); err != nil {
		log.Errorf("[dungeonactor] AsyncCall send failed: sessionId=%s msgId=%d err=%v", sessionId, msgId, err)
		return err
	}
	return nil
}
