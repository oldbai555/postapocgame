...
func (ms *MoveSys) syncPositionToGameServer(scene iface.IScene, pos *argsdef.Position) {
	...
	// 发送坐标同步消息给 PlayerActor
	sendSyncPositionToPlayerActor(sessionId, roleId, scene.GetSceneId(), pos.X, pos.Y)
}

// HandleStartMove C2S 协议处理函数（Actor 消息版本）
// 约定：msg.Context 中包含 "session" 字段，可通过 EntityMgr 根据 sessionId 查到实体。
func HandleStartMove(msg actor.IActorMessage) error {
	if msg == nil {
		return nil
	}
...
func HandleUpdateMove(msg actor.IActorMessage) error {
	if msg == nil {
		return nil
	}
...
func HandleEndMove(msg actor.IActorMessage) error {
	if msg == nil {
		return nil
	}
...
// HandleGetNearestMonster 处理获取最近怪物请求（C2S → DungeonActor）
func HandleGetNearestMonster(msg actor.IActorMessage) error {
	if msg == nil {
		return nil
	}
...
func HandleChangeScene(msg actor.IActorMessage) error {
	if msg == nil {
		return nil
	}
...
// sendSyncPositionToPlayerActor 发送坐标同步消息给 PlayerActor
func sendSyncPositionToPlayerActor(sessionId string, roleId uint64, sceneId uint32, posX uint32, posY uint32) {
	req := &protocol.D2GSyncPositionReq{
		SessionId: sessionId,
		RoleId:    roleId,
		SceneId:   sceneId,
		PosX:      posX,
	...
	if err != nil {
		log.Errorf("[move_sys] marshal D2GSyncPositionReq failed: %v", err)
		return
	}

	ctx := context.Background()
	ctx = context.WithValue(ctx, gshare.ContextKeySession, sessionId)
	actorMsg := actor.NewBaseMessage(ctx, uint16(protocol.PlayerActorMsgId_PlayerActorMsgIdSyncPosition), data)
	if err := gshare.SendMessageAsync(sessionId, actorMsg); err != nil {
		log.Errorf("[move_sys] send SyncPosition message to PlayerActor failed: sessionId=%s err=%v", sessionId, err)
	}
}
