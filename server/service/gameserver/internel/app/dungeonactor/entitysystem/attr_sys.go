...
func (as *AttrSys) syncBackToGameServer(syncData *protocol.SyncAttrData) {
	...
	// 只处理角色实体
	if as.owner.GetEntityType() != uint32(protocol.EntityType_EtRole) {
		return
	}

	// 类型断言为RoleEntity
	roleEntity, ok := as.owner.(interface {
		GetSessionId() string
		GetRoleId() uint64
	})
	if !ok {
	...
	// 发送属性同步消息给 PlayerActor
	sendSyncAttrsToPlayerActor(sessionId, roleId, syncData)
}

// sendSyncAttrsToPlayerActor 发送属性同步消息给 PlayerActor
func sendSyncAttrsToPlayerActor(sessionId string, roleId uint64, syncData *protocol.SyncAttrData) {
	...
	if err != nil {
		log.Errorf("[attr_sys] marshal D2GSyncAttrsReq failed: %v", err)
		return
	}

	ctx := context.Background()
	ctx = context.WithValue(ctx, gshare.ContextKeySession, sessionId)
	actorMsg := actor.NewBaseMessage(ctx, uint16(protocol.PlayerActorMsgId_PlayerActorMsgIdSyncAttrs), data)
	if err := gshare.SendMessageAsync(sessionId, actorMsg); err != nil {
		log.Errorf("[attr_sys] send SyncAttrs message to PlayerActor failed: sessionId=%s err=%v", sessionId, err)
	}
}
