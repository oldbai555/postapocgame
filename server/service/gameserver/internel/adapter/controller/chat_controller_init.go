...
func init() {
	gevent2.Subscribe(gevent2.OnSrvStart, func(ctx context.Context, _ *event.Event) {
		...
		clientprotocol.Register(uint16(protocol.C2SProtocol_C2SChatWorld), chatController.HandleWorldChat)
		clientprotocol.Register(uint16(protocol.C2SProtocol_C2SChatPrivate), chatController.HandlePrivateChat)
	})
}
