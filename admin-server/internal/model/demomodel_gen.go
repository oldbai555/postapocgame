...
var (
	...
	cacheDemoIdPrefix = "cache:demo:id:"
)

type (
	demoModel interface {
		Insert(ctx context.Context, data *Demo) (sql.Result, error)
		FindOne(ctx context.Context, id uint64) (*Demo, error)
		Update(ctx context.Context, data *Demo) error
		Delete(ctx context.Context, id uint64) error
		FindPage(ctx context.Context, page, pageSize int64) ([]Demo, int64, error)
		FindChunk(ctx context.Context, limit int64, lastId uint64) ([]Demo, uint64, error)
	}
...
func (m *defaultDemoModel) Delete(ctx context.Context, id uint64) error {
	demoIdKey := fmt.Sprintf("%s%v", cacheDemoIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		// 检查是否有 deleted_at 字段：通过检查结构体字段名列表
		fieldNames := builder.RawFieldNames(&Demo{})
...
func (m *defaultDemoModel) FindOne(ctx context.Context, id uint64) (*Demo, error) {
	// 检查是否有 deleted_at 字段
	hasDeletedAt := strings.Contains(demoRows, "deleted_at")
	...
	demoIdKey := fmt.Sprintf("%s%v", cacheDemoIdPrefix, id)
	var resp Demo
	err := m.QueryRowCtx(ctx, &resp, demoIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
		query := fmt.Sprintf("select %s from %s %s limit 1", demoRows, m.table, whereClause)
...
func (m *defaultDemoModel) Insert(ctx context.Context, data *Demo) (sql.Result, error) {
	...
	demoIdKey := fmt.Sprintf("%s%v", cacheDemoIdPrefix, data.Id)
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		// 手动构建包含 created_at、updated_at 的插入语句
		// 如果表有 deleted_at 字段，它已经在 RowsExpectAutoSet 中，不需要重复添加
		query := fmt.Sprintf("insert into %s (%s, `created_at`, `updated_at`) values (?, ?, ?, ?, ?)", m.table, demoRowsExpectAutoSet)
		return conn.ExecCtx(ctx, query, data.Name, data.Status, data.DeletedAt, data.CreatedAt, data.UpdatedAt)
	}, demoIdKey)
	return ret, err
}

func (m *defaultDemoModel) Update(ctx context.Context, data *Demo) error {
	demoIdKey := fmt.Sprintf("%s%v", cacheDemoIdPrefix, data.Id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		// 自动设置更新时间
		data.UpdatedAt = time.Now().Unix()
		// 检查是否有 deleted_at 字段
		hasDeletedAt := strings.Contains(demoRowsWithPlaceHolder, "deleted_at")
		// 手动构建包含 updated_at 的更新语句
		whereClause := "where `id` = ?"
		if hasDeletedAt {
			whereClause += " and deleted_at = 0"
		}
		query := fmt.Sprintf("update %s set %s, `updated_at` = %d %s", m.table, demoRowsWithPlaceHolder, data.UpdatedAt, whereClause)
		return conn.ExecCtx(ctx, query, data.Name, data.Status, data.DeletedAt, data.Id)
	}, demoIdKey)
	return err
}

func (m *defaultDemoModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheDemoIdPrefix, primary)
}
...
func (m *defaultDemoModel) queryPrimary(ctx context.Context, conn sqlx.SqlConn, v, primary any) error {
	...
	query := fmt.Sprintf("select %s from %s where `id` = ?%s limit 1", demoRows, m.table, andClause)
...
// 分页查询：根据页码和每页数量查询数据
func (m *defaultDemoModel) FindPage(ctx context.Context, page, pageSize int64) ([]Demo, int64, error) {
	if page <= 0 {
		page = 1
	}
	if pageSize <= 0 {
	...
	// 查询分页数据
	var list []Demo
	query := fmt.Sprintf("select %s from %s %s order by id desc limit %d offset %d", demoRows, m.table, whereClause, pageSize, offset)
...
func (m *defaultDemoModel) FindChunk(ctx context.Context, limit int64, lastId uint64) ([]Demo, uint64, error) {
	if limit <= 0 {
		limit = 20
	}
	if limit > 100 {
	...
	if lastId == 0 {
		// 第一次查询
		query := fmt.Sprintf("select %s from %s %s order by id asc limit %d", demoRows, m.table, whereClause, limit)
		err = m.QueryRowsNoCacheCtx(ctx, &list, query)
	} else {
		// 基于lastId的分片查询
		andClause := ""
		...
		query := fmt.Sprintf("select %s from %s %s order by id asc limit %d", demoRows, m.table, andClause, limit)
